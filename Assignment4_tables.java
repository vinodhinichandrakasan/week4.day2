...
public class Assignment4_tables {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			System.out.println("output Learn to interact with Elements" + str);
		}
		// Check the vital task for the least completed progress.
		List<WebElement> element2 = driver.findElements(By.xpath("//tbody/tr/td[2]"));
		for (int i = 1; i < element2.size(); i++) {
			String str = element2.get(i).getText();
			System.out.println("least completed progress is " + str);
			String trim = str.substring(0, 2);
			int number = Integer.parseInt(trim);
			System.out.println(number);
					
			  if(number<30) {
				  
			  driver.findElement(By.xpath("//tbody/tr["+i+"]/td[3]/input")).click();
